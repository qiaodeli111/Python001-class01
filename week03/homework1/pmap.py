...
def scan_ip_range(max_processes, ip_range):
    q = mp.Queue(100)
    flag = mp.Value('i', 0)
    ...
    def process_ping(queue: mp.Queue):
        ...
        while True:
            ...
            if queue.empty() and flag:
                ...
                if result.find('3 packets transmitted, 0 packets received, 100.0% packet loss') < 0:
...
def scan_ports(max_processes, ip_addr):
...
if __name__ == '__main__':
    ...
    if args.fangfa == 'ping':
        scan_ip_range(args.num, args.ipaddr)
    else:
        scan_ports(args.num, args.ipaddr)
